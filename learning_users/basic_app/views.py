...
@login_required
def user_logout(request):#decorator coz we need that same user to logout
    logout(request)
...
def register(request):
    ...
    if request.method == 'POST':
        ...
        if user_form.is_valid() and profile_form.is_valid():
            ...
            profile.user = user #shows onetoone relationship
...
def user_login(request):
    if request.method == 'POST':
        username = request.POST.get('username')# from name in urls
